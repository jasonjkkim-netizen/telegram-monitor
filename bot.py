...
[PERF] Reduced memory: reuse compiled regex, **slots** on hot classes
...
KST = timezone(timedelta(hours=9))  # v8.5: Korean Standard Time for market hours check
...
class KISApi:
**slots** = (‘access_token’, ‘token_expires’, ‘session’, ‘_semaphore’)
    ...
    async with self._semaphore:  # v7.9: rate limit
        try:
            ...
            if data.get("rt_cd") == "0":
                ...
                # v8.5: Debug - check if data is in 'output' instead
                if not items:
                    items = data.get("output", [])
                    if items:
                        print(f"🔍 분봉 [{stock_code}] data in 'output' not 'output2'")
                ...
                # v8.5: Debug logging when volume is 0 but items exist
                if total == 0 and items:
                    sample = items[0] if items else {}
                    print(f"🔍 분봉 [{stock_code}] volume=0 but {len(items)} candles, keys: {list(sample.keys())[:10]}")
                    print(f"🔍 분봉 [{stock_code}] candle[0] sample: {dict(list(sample.items())[:6])}")
                return total
    ...
    async with self._semaphore:
        try:
            ...
            if data.get("rt_cd") == "0":
                ...
                if not output2:
                    # v8.5: Debug - check if data is in 'output' instead of 'output2'
                    output1 = data.get("output", [])
                    if output1:
                        print(f"🔍 투자자추정 [{stock_code}] data in 'output' not 'output2', keys={list(data.keys())}")
                        output2 = output1 if isinstance(output1, list) else [output1]
                    else:
    ...
    async with self._semaphore:
        try:
            ...
            if data.get("rt_cd") == "0":
                ...
                if not output:
                    # v8.5: Debug - check 'output2'
                    output2 = data.get("output2", [])
                    if output2:
                        print(f"🔍 프로그램매매 [{stock_code}] data in 'output2' not 'output', keys={list(data.keys())}")
                        output = output2
                    else:
...
**slots** = (‘cooldown’, ‘last_alert’, ‘_last_clean_time’, ‘_lock’)
...
**slots** = (‘cooldown’, ‘last_alert’, ‘_lock’)
...
while not shutdown_event.is_set():
    try:
        ...
        # v8.5: Use explicit KST timezone instead of server local time
        now_kst = datetime.now(KST)
        hour_min = now_kst.hour * 100 + now_kst.minute
