...
====================================
1) All unique messages -> TARGET_CHANNEL
2) 실적/공시 keyword messages -> EARNINGS_CHANNEL
...
EARNINGS_KEYWORDS = [
    ...
    "실적", "잠정실적", "실적발표", "실적공시", "실적추정", "실적전망",
        ...
        # 공시 관련
        "공시", "전자공시", "DART", "dart",
        "주요사항보고서", "수시공시", "공정공시", "자율공시",
        "유상증자", "무상증자", "감자", "주식분할", "주식병합",
        "전환사채", "CB발행", "신주인수권", "BW발행",
        "자기주식", "자사주", "자사주매입", "자사주소각",
        "배당", "배당금", "중간배당", "특별배당", "배당락",
        "최대주주변경", "대주주변경", "경영권분쟁",
        "합병", "인수", "M&A", "분할", "물적분할", "인적분할",
...
# ============================================================
# DUPLICATE DETECTOR (v4.0 - improved thresholds)
# ============================================================
class DuplicateDetector:
    ...
    def is_duplicate(self, text):
        ...
        # similarity check (only for messages > 50 chars to reduce false positives)
...
async def main():
    ...
    print("  + 실적/공시 Filter + Volume Alert")
    print("  + Fixed peer ID matching")
    ...
    if not EARNINGS_CHANNEL:
                print("⚠️ EARNINGS_CHANNEL not set — 실적/공시 filter disabled\n")
    ...
    # --------------------------------------------------------
    # Build monitored IDs using utils.get_peer_id for consistency
    # --------------------------------------------------------
    ...
    async def print_stats():
                while True:
                                ...
                                print(f"\n📊 Stats: total={stats['total']} unique={stats['unique']} "
    ...
    async def handler(event):
            ...
            # Handle media-only messages (no text)
            ...
            # 2) 실적/공시 filter -> EARNINGS_CHANNEL
            ...
            # 3) Volume alert -> VOLUME_ALERT_CHANNEL
            if kis and msg:
                ...
                for code in codes[:3]:
                    ...
                    if daily_ok or fivemin_ok:
                        ...
                        try:
                                                        await client.send_message(
                                                                                            VOLUME_ALERT_CHANNEL, alert, link_preview=False
                                                        )
                                                        print(f"  🚨 ALERT SENT! ✅")
                        except Exception as e:
