"""
Telegram Channel & Group Monitor v7.6
======================================
...
v7.6 Changes:
  - Fixed DuplicateDetector: replaced O(n²) SequenceMatcher with simhash for scalable dedup
  - Added asyncio.Lock to DuplicateDetector for coroutine safety
  - Fixed stock name ISIN prefix stripping (e.g. "KR7079550000LIG넥스원" -> "LIG넥스원")
  - Tightened CRYPTO_KEYWORDS to reduce false positives on generic English words
  - Removed dead KIS_ACCOUNT_NO / KIS_ACCOUNT_PROD config
  - Added periodic heartbeat logging (every 10 min) with uptime and connection status
...
v6.1 Changes:
  - Changed BOT 3 volume check from cumulative daily (acml_tr_pbmn)
    to rolling 20-minute window using minute-candle API (FHKST03010200)
...
# Tightened: removed generic English words (coin, token, wallet, mining)
# that cause false positives in stock news context
# ============================================================
CRYPTO_KEYWORDS = [
...
class StockUniverse:
    """
    Downloads and parses KIS master files (kospi + kosdaq)
    to build a complete name->code mapping for ALL listed Korean stocks.
...
def contains_earnings_keyword(text):
    ...
    2) Bypass: high-confidence keywords (영업이익, 순이익, etc.)
       with a number pattern strongly indicate financial data
       even without an anchor keyword.
    """
    if not text:
        return False
    lower = text.lower()
...
def extract_stock_name(raw_name, stock_code):
    """
    Extract clean stock name from KIS API response.
    KIS sometimes returns ISIN-style names like 'KR7079550000LIG넥스원'.
    This strips the ISIN prefix to return just the stock name.
    """
    if not raw_name or raw_name == stock_code or raw_name.isdigit():
        return None
    # Strip ISIN prefix pattern: KR + 1 digit + 6-digit code + check digits + name
...
# ============================================================
# SIMHASH-BASED DUPLICATE DETECTOR (v7.6 - replaces O(n²) SequenceMatcher)
# ============================================================
def _simhash(text, hashbits=64):
    """
    Compute a simhash fingerprint for the given text.
    Uses character-level 3-grams (shingles) for robustness.
...
def _hamming_distance(a, b, hashbits=64):
...
class DuplicateDetector:
    """
    Scalable duplicate detector using exact hash + simhash.
    - Exact duplicates: caught by MD5 hash (O(1) lookup)
    - Near duplicates: caught by simhash with Hamming distance threshold (O(n) but fast bitwise ops)
    ...
    def __init__(self, threshold=0.85, max_history=500, ttl_hours=6):
        ...
        # Convert similarity threshold (0.0-1.0) to Hamming distance threshold
        # simhash uses 64 bits, so threshold 0.85 means max ~10 bits different
        self.hamming_threshold = int((1 - threshold) * 64)
        self.seen_hashes = {}       # {md5_hash: timestamp}
        self.seen_simhashes = []    # [(simhash, timestamp)]
    ...
    async def is_duplicate(self, text):
        """Check if text is a duplicate. Coroutine-safe via asyncio.Lock."""
        async with self._lock:
            ...
            # 1) Exact hash check (O(1))
            text_hash = self._get_hash(text)
            if text_hash in self.seen_hashes:
                self.stats["duplicate"] += 1
                return True

            # 2) Simhash near-duplicate check (O(n) but fast bitwise ops)
...
async def main():
    ...
    async def print_stats_and_heartbeat():
        """Periodic stats + heartbeat logging (every 10 min) with uptime and connection status."""
        try:
            while True:
                ...
                print(
                    f"\U0001f49a Heartbeat: uptime={uptime_h}h{uptime_m}m | "
    ...
    bg_tasks.append(asyncio.create_task(print_stats_and_heartbeat()))
    ...
    async def handler(event):
        try:
            ...
            if msg:
                ...
                if await detector.is_duplicate(msg):
                    return
            ...
            if EARNINGS_CHANNEL and (msg or ocr_text) and contains_earnings_keyword(combined_text):
                matched = [
                    kw for kw in EARNINGS_ANCHOR_KEYWORDS + EARNINGS_DATA_KEYWORDS + HIGH_CONFIDENCE_EARNINGS_KEYWORDS
            ...
            if kis and combined_text:
                ...
                for code in codes:
                    ...
                    # v7.6: Extract clean name, stripping ISIN prefix
                    cleaned_api_name = extract_stock_name(api_name, code)
                    if cleaned_api_name:
                        name = cleaned_api_name
                    elif universe_name:
                        name = universe_name
                    else:
                        name = code
                    ...
                    print(
                        f"  \U0001f4b0 {source_type}[{code}] {name} {price_info['price']:,}원 "
                    ...
                    if w52h and w52l:
                        ...
                        alert_lines.append(
                            f"\U0001f4cd 52주: {w52l:,} ~ {w52h:,} (현재 {pos_pct:.0f}% 위치)"
                        )
                    ...
                    alert_lines.extend([
                        ...
                        + (
                            f" \U0001f50d(OCR)"
                            if ocr_text and code not in (
                                stock_universe.find_stocks_in_text(msg) if msg else []
                            )
                            else ""
                        ),
                    ...
                    alert = "\n".join(alert_lines)
                    try:
                        await safe_send(client, VOLUME_ALERT_CHANNEL, alert, link_preview=False)
    ...
    print(f"\U0001f3a7 Listening... (v7.6)")
