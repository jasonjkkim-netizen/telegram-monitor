"""
Telegram Channel & Group Monitor v5.0
======================================
...
v5.0 Fixes:
- Added CRYPTO_KEYWORDS filter to block all crypto/coin references in BOT 1
- Fixed KIS API base URL construction (was missing base URL prefix)
- Fixed 5-minute volume API: using correct endpoint + field name (stck_cntg_hour)
- Fixed volume threshold: BOT 3 uses 10억원 (1,000,000,000 won) realtime
- Fixed contains_earnings_keyword: now correctly checks kw.lower() in text.lower()
- Fixed EARNINGS_KEYWORDS to use .lower() in keyword checking
- Fixed handler to skip messages with crypto keywords before dedup/forward
- Fixed missing await on get_five_min_volume return (was returning None on error)
- Added KIS_ACCOUNT_NO usage in headers where required
- Fixed token URL to use full base URL
- Fixed session URL to prepend KIS_BASE_URL correctly
"""
...
# 누적 거래대금 보조 기준 (100억원)
DAILY_VOLUME_THRESHOLD = int(os.environ.get("DAILY_VOLUME_THRESHOLD", "10000000000"))

KIS_BASE_URL = "https://openapi.koreainvestment.com:9443"

# ============================================================
# CRYPTO FILTER KEYWORDS (BOT 1 - 이 키워드 포함 메시지는 무시)
# ============================================================
CRYPTO_KEYWORDS = [
        # English
    "bitcoin", "btc", "ethereum", "eth", "crypto", "cryptocurrency",
        "blockchain", "defi", "nft", "altcoin", "altcoins", "coin", "coins",
        "token", "tokens", "binance", "coinbase", "upbit", "bithumb", "korbit",
        "ripple", "xrp", "solana", "sol", "dogecoin", "doge", "usdt", "usdc",
        "stablecoin", "web3", "metaverse", "airdrop", "staking", "mining",
        "wallet", "ledger", "metamask", "uniswap", "pancakeswap",
        # Korean
        "비트코인", "이더리움", "가상화폐", "암호화폐", "코인", "토큰",
        "블록체인", "디파이", "엔에프티", "알트코인", "업비트", "빗썸",
        "리플", "솔라나", "도지코인", "스테이킹", "채굴", "지갑",
        "코인마켓", "가상자산", "디지털자산", "바이낸스",
]
...
class KISApi:
        ...
        async def ensure_session(self):
                    if self.session is None or self.session.closed:
                                    self.session = aiohttp.ClientSession(base_url=KIS_BASE_URL)
    ...
    async def get_realtime_volume(self, stock_code):
                """
                        실시간(당일 최근 체결) 거래대금 조회.
                                FHKST03010200 = 국내주식 분봉 조회 -> output2 리스트의 최신 체결 데이터.
                                        최근 1개 봉의 cntg_vol * stck_prpr 를 실시간 거래대금 proxy 로 사용.
                                                더 정확한 실시간은 acml_tr_pbmn(누적)을 사용하는 get_stock_price() 참고.
                                                        """
        token = await self.get_token()
        if not token:
                        return 0
...
def contains_crypto_keyword(text):
        """메시지가 크립토/코인 관련이면 True 반환 → BOT 1에서 제외"""
    if not text:
                return False
    lower = text.lower()
...
async def main():
    ...
    async def print_stats():
            ...
            print(f"📊 Stats: total={stats['total']} unique={stats['unique']} dup={stats['duplicate']} skip={stats['skipped']}")
    ...
    async def handler(event):
            ...
            # ── BOT 1: 텍스트 메시지 ────────────────────────────────
            # [FIX] 크립토/코인 관련 메시지 필터링
            if contains_crypto_keyword(msg):
                                print(f"🚫 [{chat_name}] Crypto keyword found — skipping")
                                return
            ...
            # ── BOT 3: 거래대금 알림 ────────────────────────────────
                if kis and msg:
                                    ...
                                    for code in codes:
                                                            ...
                                                            daily = price_info["acml_tr_pbmn"]
                                                            daily_ok = daily >= DAILY_VOLUME_THRESHOLD

                                        # 실시간 5분 거래대금
                                                            realtime = await kis.get_realtime_volume(code)
                                                            realtime_ok = realtime >= REALTIME_VOLUME_THRESHOLD

                                        print(f"  💰 [{code}] 누적:{daily/100000000:.1f}억 실시간:{realtime/100000000:.2f}억")

                    # [FIX] 기준: 실시간 거래대금 >= 10억원 (OR 누적 >= 100억)
                    if realtime_ok or daily_ok:
                                                reasons = []
                                                if realtime_ok:
                                                                                reasons.append(f"실시간 {realtime/100000000:.1f}억")
                                                                            if daily_ok:
                                                    reasons.append(f"누적 {daily/100000000:.0f}억")

                                                                                                        alert = (
                                                                                                            f"🚨 **거래대금 매수신호 알림**\n"
                                                                                                                                        f"📌 **{name}** ({code})\n"
                                                                                                                                        f"💰 현재가: {price_info['price']:,}원 ({price_info['change_rate']}%)\n"
                                                                                                                                        f"📊 {' | '.join(reasons)}\n"
